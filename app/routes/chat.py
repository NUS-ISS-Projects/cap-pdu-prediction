...
def generate_analysis(data, time_unit, periods):
    ...
    if total_packets == 0:
        return f"No PDU activity detected in the past {periods} {time_unit}(s). The system appears to be inactive during this period."
    ...
    analysis += f"• **Entity State PDUs:** {entity_state:,} ({entity_state/total_packets*100:.1f}%)\n" if total_packets > 0 else ""
    ...
    if time_unit in ['week', 'month'] and len(buckets) > 1:
        # Find peak period
        peak_desc = peak_bucket.get('week', peak_bucket.get('date', peak_bucket.get('hour', 'Unknown')))
        analysis += f"\n• **Peak Activity:** {peak_packets:,} packets during {peak_desc}\n"

    # Add insights
    if entity_state > total_packets * 0.8:
        analysis += f"\n💡 **Insight:** High entity state activity suggests active simulation with many moving objects."
    if fire_events > 0:
        analysis += f"\n🔥 **Combat Activity:** {fire_events} fire events detected during this period."
    if collisions > 0 or detonations > 0:
        analysis += f"\n💥 **Impact Events:** {collisions + detonations} collision/detonation events recorded."
...
def chat():
    try:
        ...
        try:
            historical_data = rag._fetch_historical_data(
            ...
            answer = "Unable to fetch data from the acquisition service. Please ensure the service is running and accessible."

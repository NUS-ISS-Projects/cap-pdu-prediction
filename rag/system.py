"""RAG (Retrieval-Augmented Generation) system for DIS PDU data."""
...
class DISDataRAGSystem:
    """RAG system for retrieving and processing DIS PDU data."""
    ...
    def set_jwt_token(self, token: str):
        """Set JWT token and trigger initial data refresh."""
    ...
    def refresh_data_cache(self):
        """Refresh cached data from the acquisition service."""
        print("Refreshing data cache...")

        realtime_data = self.client.fetch_realtime_data()
        print(
            f"Real-time data fetched: {bool(realtime_data)}, "
            f"keys: {list(realtime_data.keys()) if realtime_data else 'None'}"
        )
        ...
        cache_updates = {
            ...
            "aggregated_today": self.client.fetch_historical_data(
                start_of_today, end_of_today, "hour"
            ),
            "aggregated_week": self.client.fetch_historical_data(
                (today - timedelta(days=7)).strftime("%Y-%m-%d"), today.strftime("%Y-%m-%d"), "day"
            ),
            "aggregated_month": self.client.fetch_historical_data(
                (today - timedelta(days=30)).strftime("%Y-%m-%d"),
                today.strftime("%Y-%m-%d"),
                "week",
            ),
            "historical_week": self.client.fetch_historical_data(
                (today - timedelta(days=7)).strftime("%Y-%m-%d"), today.strftime("%Y-%m-%d"), "day"
            ),
            "daily_pdu_logs": self.client.fetch_pdu_logs(
                current_time_ms - (24 * 60 * 60 * 1000), current_time_ms
            ),
            "weekly_pdu_logs": self.client.fetch_pdu_logs(
                current_time_ms - (7 * 24 * 60 * 60 * 1000), current_time_ms
            ),
            "monthly_pdu_logs": self.client.fetch_pdu_logs(
                current_time_ms - (30 * 24 * 60 * 60 * 1000), current_time_ms
            ),
        }
    ...
    def fetch_historical_data(
        ...
        data = self.client.fetch_historical_data(start_date, end_date, time_unit)
        print(f"Fetched {len(data.get('buckets', []))} data points " "from acquisition service")
        return data
    ...
    def _build_vector_store(self):
        """Build vector store from cached data chunks."""
    ...
    def answer_question(self, question: str) -> str:
        """Answer a question using RAG over cached PDU data."""
        ...
        if not self.vector_store.texts:
            return (
                ...
                "Please ensure the data acquisition service is running and try again."
            )
        ...
        return (
            f"Based on the available DIS PDU data:\n\n{context}\n\n"
            f"This information should help answer your question about: {question}"
        )
